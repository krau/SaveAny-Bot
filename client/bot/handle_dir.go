...
// 	dirs, err := database.GetUserDirsByChatID(userChatID)
// 	if err != nil {
// 		log.FromContext(ctx).Errorf("获取用户路径失败: %s", err)
...
// 	}
// 	user, err := database.GetUserByChatID(update.GetUserChat().GetID())
// 	if err != nil {
// 		log.FromContext(ctx).Errorf("获取用户失败: %s", err)
...
// func addDir(ctx *ext.Context, update *ext.Update, user *database.User, storageName, path string) error {
// 	if _, err := storage.GetStorageByUserIDAndName(ctx, user.ChatID, storageName); err != nil {
// 		ctx.Reply(update, ext.ReplyTextString(err.Error()), nil)
// 		return dispatcher.EndGroups
// 	}

// 	if err := database.CreateDirForUser(user.ID, storageName, path); err != nil {
// 		log.FromContext(ctx).Errorf("创建路径失败: %s", err)
...
// 	if err := database.DeleteDirByID(uint(dirID)); err != nil {
// 		log.FromContext(ctx).Errorf("删除路径失败: %s", err)
